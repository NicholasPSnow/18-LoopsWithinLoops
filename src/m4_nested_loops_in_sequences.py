...
def multiply_numbers(sequence_of_lists):
    ...
    for k in range(len(sequence_of_lists)):
        for n in range(0,len(sequence_of_lists[k])):
            sequence_of_lists[k][n]=sequence_of_lists[k][n]*(k+1)
...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 0
    answer = sum_numbers([(0,0,0), (0, 0), [0, 0, 0, 0]])
    print('Expected and actual are:', expected, answer)
...
def sum_numbers(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for n in range(len(seq_seq[k])):
            total=total+seq_seq[k][n]
    return total
...
def print_characters(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        for n in range(len(sequence_of_strings[k])):
            print(sequence_of_strings[k][n])
